...
export const InfoField = forwardRef<HTMLInputElement, InfoFieldProps>(
	({ disabled, label, defaultValue, editable, ...other }, forwardedRef) => (
		<div className="flex items-baseline gap-2">
			<p className="w-24 flex-shrink-0 text-blue-500">{label}</p>
			<div className="flex-1">
				<input
					...
					placeholder="—"
					className="w-full rounded border px-2 py-1 transition-colors enabled:placeholder:text-transparent disabled:border-transparent"
					{...other}
				/>
			</div>
		</div>
